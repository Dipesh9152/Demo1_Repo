...
<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:math="http://exslt.org/math" xmlns:testng="http://testng.org">
    <xsl:output method="html" indent="yes" omit-xml-declaration="yes" doctype-public="-//W3C//DTD XHTML 1.0 Transitional//EN" doctype-system="http://www.w3.org/TR/xhtml1/DTD/xhtml1-transitional.dtd" />
    ...
    <xsl:template name="writeCssFile">
        <xsl:result-document href="{testng:absolutePath('style.css')}" format="text">
            ...
            body { font-family: Arial, sans-serif; font-size: 12px; padding: 10px; margin: 0px; background:white url(../fronter_logo.gif) no-repeat scroll top right; }
    ...
    <xsl:template name="writeJsFile">
        <xsl:result-document href="{testng:absolutePath('main.js')}" format="text"><![CDATA[
    ...
    <xsl:function name="testng:suiteMethodsCount">
        ...
        <xsl:value-of select="if ($state = '*') then count($testCasesElements/class/test-method[not(@is-config)]) else count($testCasesElements/class/test-method[(@status=$state) and (not(@is-config))])" />
    </xsl:function>
    ...
    <xsl:template match="/testng-results">
        ...
        <xsl:result-document href="{testng:absolutePath('overview-chart.svg')}" format="xml">
            <svg xmlns="http://www.w3.org/2000/svg" preserveAspectRatio="xMidYMid meet" width="{$chartWidth}" height="{$chartHeight}" viewBox="0 0 900 300">
                <defs>
                ...
                <svg id="graphzone" preserveAspectRatio="xMidYMid meet" x="0" y="0">
                    <xsl:variable name="testCaseElements" select="                     if (suite/@url) then                         if (document(suite/@url)/suite/test/@url)                             then document(document(suite/@url)/suite/test/@url)/test                              else document(suite/@url)/suite/test                         else suite/test" />
                    ...
                    <rect style="fill:red;stroke-width:1;stroke:black;" x="10" y="50" width="20" height="20" />
                    <text class="legendtext" x="40" y="70">Failed (<xsl:value-of select="$failedPercent" />)
                    </text>
                    <rect style="fill:green;stroke-width:1;stroke:black;" x="10" y="80" width="20" height="20" />
                    <text class="legendtext" x="40" y="100">Passed (<xsl:value-of select="$passPercent" />)
                    </text>
                    <rect style="fill:yellow;stroke-width:1;stroke:black;" x="10" y="110" width="20" height="20" />
                    <text class="legendtext" x="40" y="130">Skipped (<xsl:value-of select="$skipPercent" />)
                    </text>
                    <g style="stroke:black;stroke-width:1" transform="translate(450,150)">
                        ...
                        <xsl:if test="($failedCount div $totalCount) &gt; 0">
                            <g style="fill:red" transform="rotate(-{$failedRotation}) translate({round($failedX_text div 4)}, {round($failedY_text div 4)})">
        ...
        <xsl:result-document href="{testng:absolutePath('overview.html')}" format="xhtml">
            <html xmlns="http://www.w3.org/1999/xhtml">
                <xsl:call-template name="htmlHead" />
                <body>
                    <h2>Test Suite Result on <xsl:value-of select="current-date()" /></h2>
                    <table width="100%">
                        <tr>
                    ...
                    <table width="100%" cellpadding="5" cellspacing="1">
                        <tr style="background-color: #f7eaf0;">
                            <td width="100%">
                                <div class="suiteStatus" title="Individual Suite Status" />Auction Automation</td>
                            <td style="font-size: 14; background-color: #FFBBBB; padding: 3px 3px 3px 0;" align="center">
                                <div style="width: 50px;">Failed</div>
                            </td>
                            <td style="font-size: 14; background-color: lightgreen; padding: 3px 3px 3px 0;" align="center">
                                <div style="width: 50px;">Passed</div>
                            </td>
                            <td style="font-size: 14; background-color: #FFFFBB; padding: 3px 3px 3px 0;" align="center">
                                <div style="width: 50px;">Skipped</div>
                            </td>
                            <td align="center" style="font-size: 14; background-color: #eaf0f7; padding: 3px 3px 3px 0;">
                                <div style="width: 50px;">Total</div>
                            </td>
                            <td align="center" style="font-size: 13; background-color: #eaf0f7; padding: 3px 3px 3px 0;">
                                <div style="width: 50px;">Pass(%)</div>
                            </td>
                            <xsl:if test="compare($testNgXslt.showRuntimeTotals, 'true') = 0">
                                <td style="font-size: 14; background-color: #eaf0f7; padding: 3px 3px 3px 0;" align="center">
                        ...
                        <tr style="background-color: #f5f5f5; font-size: 15px;">
                            ...
                            <xsl:variable name="suiteElements" select="if (suite/@url) then document(suite/@url)/suite else suite" />
                            <xsl:variable name="totalTime" select="testng:formatDuration(sum($suiteElements/@duration-ms))" />
                            <xsl:variable name="testCaseElements" select="                                             if (suite/@url) then                                                 if (document(suite/@url)/suite/test/@url)                                                     then document(document(suite/@url)/suite/test/@url)/test                                                      else document(suite/@url)/suite/test                                                 else suite/test" />
                            <xsl:variable name="failedCount" select="testng:suiteMethodsCount($testCaseElements, 'FAIL')" />
                            <xsl:variable name="passedCount" select="testng:suiteMethodsCount($testCaseElements, 'PASS')" />
                            <xsl:variable name="skippedCount" select="testng:suiteMethodsCount($testCaseElements, 'SKIP')" />
                            <xsl:variable name="totalCount" select="testng:suiteMethodsCount($testCaseElements, '*')" />
                            <xsl:variable name="passedPercent" select="format-number($passedCount div $totalCount, '###%')" />
                            <td align="center">
                        ...
                        <table width="100%" cellpadding="5" cellspacing="1">
                            <tr style="background-color: #eaf0f7;">
                                ...
                                <xsl:call-template name="percentageOverview">
                                    ...
                                    <xsl:with-param name="skippedCount" select="testng:suiteMethodsCount($testCaseElements, 'SKIP')" />
        ...
        <xsl:result-document href="{testng:absolutePath('navigation.html')}" format="xhtml">
            <html xmlns="http://www.w3.org/1999/xhtml">
                ...
                <body>
                    ...
                    <div>
                        <a href="overview.html" target="content" onclick="javscript:clearAllSelections();">Results overview
                        </a>
                    </div>
                    <div>
                        <a href="reporterOutput.html" target="content" onclick="javscript:clearAllSelections();">Reporter output
                        </a>
                    </div>
                    <div>
                        <xsl:for-each select="$suiteElements">
                            ...
                            <table class="suiteMenuHeader" width="100%" cellpadding="0" cellspacing="0">
                                <tr>
                                    <td nowrap="true">
                                        <b>
                                            <a href="{testng:suiteContentFileName(.)}" target="content" onclick="javscript:clearAllSelections();">
                                                <xsl:value-of select="@name" />
                                            </a>
                                        </b>
                                        <div style="margin: 3px 0 3px 0;">
                                            <a href="{testng:suiteGroupsFileName(.)}" target="content" onclick="javscript:clearAllSelections();">
                                        ...
                                        /
                                        <span style="color: green;"><xsl:value-of select="testng:suiteMethodsCount($testCaseElements, 'PASS')" /></span>
                                        /
                                        <span style="color: yellow;"><xsl:value-of select="testng:suiteMethodsCount($testCaseElements, 'SKIP')" /></span>
                                        /
                                        <span><xsl:value-of select="testng:suiteMethodsCount($testCaseElements, '*')" /></span></td>
        ...
        <xsl:result-document href="{testng:absolutePath(testng:suiteGroupsFileName($suiteElement))}" format="xhtml">
            <html xmlns="http://www.w3.org/1999/xhtml">
                ...
                <body>
                    <h2>
                        Groups for suite:
                        <b><xsl:value-of select="$suiteElement/@name" /></b></h2>
                    <xsl:for-each select="$suiteElement/groups/group">
                        <xsl:sort order="ascending" select="@name" />
                        <table style="margin-bottom: 20px; font-size: 12px; width:100%;" cellpadding="3" cellspacing="1">
                            <tr>
    ...
    <xsl:template name="testMethodsList">
        ...
        <xsl:for-each select="$methodList">
            ...
            <tr id="{concat($methodId, '_row')}" class="{testng:testMethodStatus(.)}">
                <xsl:if test="testng:isFilterSelected(@status) != 'true'"></xsl:if>
                ...
                <td nowrap="true">
                    <xsl:if test="./exception">
                        <xsl:variable name="methodName" select="concat(@name,'.png')"/>
                        <a href="screenshots/{$methodName}" target="_blank">View screenshot</a>
                    </xsl:if>
                </td>
                <td nowrap="true">
                    <xsl:if test="./exception">
                        <a onclick="toggleDetailsVisibility('{$exceptionDetailsId}')">
                            View details
                            <xsl:if test="contains(exception/@class, 'java.lang.AssertionError')">
                                (verification point failure)
                            </xsl:if>
                            <xsl:if test="contains(exception/@class, 'org.openqa.selenium.TimeoutException')">
                                (element did not appear within timeout)
                            </xsl:if>
                            <xsl:if test="contains(exception/@class, 'org.openqa.selenium.NoSuchElementException')">
            ...
            <tr>
                <td colspan="6" class="detailsBox">
                    <div id="{$detailsId}" class="testMethodDetails">
                        ...
                        <xsl:if test="./reporter-output/line and not(@is-config)">
                            <div>
                                <table width="75%">
                                    <tbody>
                                        <tr>
                                        ...
                                        <table width="75%">
                                            <tbody>
                                                <tr>
                                                    <td width="93%">
                                                        <font size="2">
                                                            <xsl:value-of select="substring-before(text(),': Passed')" />
                                                        </font>
                                                    </td>
                                                    <td width="7%">
                                                        <b>
                                                            <font color="#00b800" size="2">Passed</font>
                                                        </b>
                                                    </td>
                                                </tr>
                                            </tbody>
                            ...
                            <!-- Added For Failed Test Cases and Exceptions -->
                        </xsl:if>
                        <xsl:if test="./exception">
                            <div>
                                <table width="75%">
                            ...
                            <xsl:for-each select="./exception/message">
                                        ...
                                        <table width="75%">
                                            <tbody>
                                                <tr>
                                                    <td width="93%">
                                                        <font size="2">
                                                            <xsl:value-of select="substring-before(text(),': Failed')" />
                                                        </font>
                                                    </td>
                                                    <td width="7%">
                                                        <b>
                                                            <font color="#FF0000" size="2">Failed</font>
                                                        </b>
                                                    </td>
                                                </tr>
                                            </tbody>
            ...
            <tr nowrap="true">
                <xsl:if test="exception">
                    <td colspan="6" class="detailsBox" nowrap="true">
                        <div id="{$exceptionDetailsId}" class="testMethodDetails">
                            <xsl:choose>
                                <xsl:when test="exception/full-stacktrace">
                                    <pre style="padding: 0px; margin: 0;">
                                        <xsl:value-of select="replace(exception/full-stacktrace, '\t+', ' ')" />
                                    </pre>
                                </xsl:when>
                                <xsl:otherwise>
    ...
    <xsl:template name="suiteTestCasesLinks">
        ...
        <xsl:for-each select="$testCases">
            ...
            <div class="testCaseLink" onclick="javscript:selectTestCaseLink(this); parent.content.location='{testng:testCaseContentFileName(.)}'">
                <div class="{if (count(./class/test-method[@status='FAIL']) &gt; 0)                                 then 'testCaseFail'                                 else if ((count(./class/test-method[@status='FAIL']) = 0) and (count(./class/test-method[@status='PASS']) &gt; 0))                                     then 'testCasePass'                                     else 'testCaseSkip'}"></div>
                <xsl:value-of select="@name" />
            </div>
        </xsl:for-each>
    </xsl:template>
    ...
    <xsl:template name="powered-by">
        <div style="margin-top: 15px; color: blue; text-align: center; font-size: 12px;">Auction Automation Report</div>
    </xsl:template>
</xsl:stylesheet>